...
def multi_minimax(depth, game_state, super_pruning=False, use_voronoi=True):
    ...
    for action in reversed(copy_actions):
        ...
        for opponent_id in min_player_ids:
            ...
            min_move = min(min_move, minimax(max_player, opponent, depth-1, max_depth, alpha, beta, False, model, node,
                                             use_voronoi))
...
def minimax(max_player, min_player, depth, max_depth, alpha, beta, is_max, model, last_parent_node, use_voronoi):
    ...
    if is_max:
        ...
        for action in list(Action):
            ...
            min_move = min(min_move, minimax(max_player, min_player, depth-1, max_depth, alpha, beta, True, model, node,
                                             use_voronoi))
